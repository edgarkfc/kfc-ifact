# server/schemas/nota_credito_schema.py
"""
Schema de Nota de Crédito - Migrado desde validators.py
"""
...
class NotaCreditoSchema(Schema):
    ...
    def unwrap_nota_credito(self, data, **kwargs):
        ...
        if isinstance(data, dict) and 'nota_credito' in data:
            logger.debug("Desempaquetando campo 'nota_credito'")
        ...
        if data.get('formas_pago') and data.get('cabecera_factura'):
            ...
            if abs(total_formas_pago_rounded - total_cabecera) > TOLERANCIA:
                errores.append(
                    ...
                    f'no coincide con el total de nota de crédito ({total_cabecera:.2f})'
                )
